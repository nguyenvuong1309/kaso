...
- [x] Paywall thông minh - Đã có `PaywallDomain` (SubscriptionTier Comparable rank free<pro<family, SubscriptionFeatureFlag 15 toggle với `minimumTier`, PricingPlan + bundledCatalogue, SubscriptionEntitlement + Repository, PaywallStoreClient abstraction với `ResolvedProduct`/`PaywallPurchaseOutcome` decouple khỏi StoreKit, `PaywallStoreClient.empty`/`.preview`); `PaywallFeature` TCA với task load entitlement + products, selectTier picker, purchaseButtonTapped → outcome routing (purchased/cancelled/pending/failed) → save entitlement, restoreButtonTapped flow, success/error banners; UI `PaywallView` + `PaywallCards` hero + tier segmented picker + feature checklist theo tier + plan rows với recommended badge + restore button + legal footer; live `LivePaywallStoreClient` trong App target dùng StoreKit 2 (`Product.products(for:)`, `product.purchase()`, `Transaction.currentEntitlements`, `AppStore.sync()` cho restore, verify check + finish transaction), tier derived từ productID; persistence mã hoá `EncryptedSubscriptionEntitlementStore` AES-GCM keychain; ghép vào `KasoRootFeature` qua `isPaywallPresented` + floating "Nâng cấp" button; full localization VI/EN cho 15 feature flag + 3 tier + cycle + status. Tests: 6 domain test + 5 TCA TestStore test cover load, tier select, purchase outcomes, restore. Smart trigger thêm `PaywallPromptScheduler` (trialPeriod 14 ngày, cooldown 7 ngày) + `PaywallPromptSchedule` model + `EncryptedPaywallPromptScheduleStore`; `KasoRootFeature.task` chạy `evaluatePaywallPrompt` mỗi launch — `recordFirstLaunchIfNeeded`, load entitlement, gọi `PaywallPromptScheduler.shouldPrompt(tier:schedule:now:)` (chỉ free tier, sau trial, không spam khi cooldown, dừng hẳn nếu user dismiss permanent) → nếu true thì auto-set `isPaywallPresented=true` và `recordPromptShown`; 8 test domain cover paid tier không prompt, trial chưa đủ, cooldown, permanent dismissal, idempotency của recordFirstLaunch. Feature gating: thêm `PaywallGate` domain helper với `PaywallGateDecision` enum (.allowed | .gated(requiresTier:)), pure function `evaluate(feature:entitlement:)` + convenience `evaluate(feature:tier:)`; `KasoRootFeature.State` lưu `currentEntitlement` + `paywallGateRequestedFeature: SubscriptionFeatureFlag?`, expose `gateDecision(for:)` helper; `evaluatePaywallPrompt` send `entitlementLoaded` action sau khi đọc entitlement repo; thêm action `proGateRequested(SubscriptionFeatureFlag)` — nếu user đã unlock thì noop, nếu gated thì set requested feature + open paywall; `paywallButtonTapped`/`paywallDismissed` reset gate context; UI demo gate cho `aiInsights`: Financial Assistant floating button hiện icon `lock.fill` thay `sparkles` khi free tier, tap → `proGateRequested(.aiInsights)` thay vì mở assistant; 5 test gate domain (free/pro/family unlocks) + 4 TCA test root (gate request mở paywall cho free, noop cho paid, dismiss clear context, gateDecision evaluate đúng theo current entitlement). Contextual hero: `PaywallFeature.State.triggeringFeature: SubscriptionFeatureFlag?` + action `setTriggeringFeature` cũng auto-pick `selectedTier = feature.minimumTier` (ví dụ vào qua `familyCompatibility` thì tier picker nhảy thẳng Family); `KasoRootFeature` gửi `.paywall(.setTriggeringFeature(feature))` khi `proGateRequested` thành công hoặc `nil` khi mở qua floating button / dismiss; `PaywallHeroCard` nếu có triggeringFeature thì hiển thị "Mở khoá [feature name]" + lock.open badge tier yêu cầu + subtitle "Tính năng này thuộc gói trả phí"; localization VI/EN cho hero gate copy. Delegate pattern cho child gating: `TransactionFeature.State.currentEntitlement` + `entitlementUpdated(SubscriptionEntitlement)` + `delegate(Delegate)` action với `Delegate.paywallGateRequested(SubscriptionFeatureFlag)`; `bankStatementImportButtonTapped`, `voiceInputButtonTapped`, `receiptImageDataSelected` đều evaluate `PaywallGate` với feature flag tương ứng (`.bankStatementImport` / `.voiceEntry` / `.ocrReceipt`) — gated thì gửi `.delegate(.paywallGateRequested(feature))` thay vì chạy logic, `KasoRootFeature` listen `.transaction(.delegate(.paywallGateRequested(feature)))` → forward thành `.proGateRequested(feature)` → mở paywall với contextual hero; `KasoRootFeature.entitlementLoaded` cũng forward `.transaction(.entitlementUpdated(entitlement))` để child reducer luôn có entitlement mới nhất.
