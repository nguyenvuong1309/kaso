...
// Two environments. Dev is the default (first) configuration so a plain
// `tuist build` targets dev. All per-environment values live in the xcconfig
// files and flow into entitlements/Info.plist via `$(VAR)` substitution.
...
let projectSettings = Settings.settings(base: baseSettings, configurations: buildConfigurations)
...
let appTarget = Target.target(
    ...
    destinations: destinations,
    product: .app,
    bundleId: "\(bundlePrefix)$(KASO_BUNDLE_SUFFIX)",
    deploymentTargets: deploymentTarget,
    infoPlist: .extendingDefault(
        with: [
            "CFBundleDisplayName": .string("$(KASO_APP_NAME)"),
            "KasoEnvironment": .string("$(KASO_ENV)"),
            "KasoAPIBaseURL": .string("$(KASO_API_BASE_URL)"),
            "KasoAppGroupIdentifier": .string("$(KASO_APP_GROUP)"),
            "KasoVerboseLogging": .string("$(KASO_VERBOSE_LOGGING)"),
    ...
    dependencies: targetDependencies([
        ...
        "GamificationFeature", "HoursOfLifeFeature", "InvestmentFeature",
        "KasoDesignSystem", "KasoFoundation", "KasoRootFeature",
...
let watchTarget = Target.target(
    ...
    product: .app,
    bundleId: "\(bundlePrefix)$(KASO_BUNDLE_SUFFIX).watchkitapp",
    ...
    infoPlist: .extendingDefault(
        with: [
            ...
            "WKCompanionAppBundleIdentifier": .string("\(bundlePrefix)$(KASO_BUNDLE_SUFFIX)"),
        ]
    ),
...
let widgetTarget = Target.target(
    ...
    bundleId: "\(bundlePrefix)$(KASO_BUNDLE_SUFFIX).widgets",
    deploymentTargets: deploymentTarget,
    infoPlist: .extendingDefault(
        with: [
            "CFBundleDisplayName": .string("$(KASO_APP_NAME) Widgets"),
            "KasoAppGroupIdentifier": .string("$(KASO_APP_GROUP)"),
...
// Explicit schemes for picking an environment when running/archiving the app.
// Per-target test schemes are still autogenerated by Tuist (they default to Dev).
let appSchemes: [Scheme] = [
    .scheme(
        name: "Kaso Dev",
        shared: true,
        buildAction: .buildAction(targets: ["Kaso"]),
